  ...
  const allRoutesSummaryPro = useMemo(() => {
    const summary = routeNames.map((name) => {
      ...
      return {
        ...
        multipacotes:
          (pts.length - uniqueStops) * pricingParams.packagePriceSamePoint,
      };
    });
                ...
                <table className="w-full table-auto text-left border-collapse bg-white break-words">
                  <thead>
                    <tr className="bg-slate-900 text-white">
                      ...
                      <th
                        className="p-2 text-[10px] font-black uppercase tracking-widest text-right cursor-pointer group hover:bg-slate-800 transition-colors whitespace-normal break-words"
                        onClick={() => handleSortPro("pesoKg")}
                      >
                        <div className="flex items-center justify-end">
                          Peso (KG){" "}
                          <SortIcon
                            active={proSortKey === "pesoKg"}
                            dir={proSortDir}
                          />
                        </div>
                      </th>
                      <th
                        className="p-2 text-[10px] font-black uppercase tracking-widest text-right cursor-pointer group hover:bg-slate-800 transition-colors whitespace-normal break-words"
                        onClick={() => handleSortPro("volumeM3")}
                      >
                        <div className="flex items-center justify-end">
                          Volume (M3){" "}
                          <SortIcon
                            active={proSortKey === "volumeM3"}
                            dir={proSortDir}
                          />
                        </div>
                      </th>
                      <th
                        className="p-2 text-[10px] font-black uppercase tracking-widest text-right cursor-pointer group hover:bg-slate-800 transition-colors whitespace-normal break-words"
                        onClick={() => handleSortPro("multipacotes")}
                      >
                        <div className="flex items-center justify-end">
                          Multipacotes (R$){" "}
                          <SortIcon
                            active={proSortKey === "multipacotes"}
                            dir={proSortDir}
                          />
                        </div>
                      </th>
                    ...
                    {allRoutesSummaryPro.map((route, idx) => (
                      ...
                      >
                        ...
                        <td className="p-1 text-right whitespace-normal break-words">
                          <span className="text-xs font-mono font-black text-slate-600 relative inline-flex">
                            <span className="peer cursor-help">
                              {formatCurrency(route.multipacotes)}
                            </span>
                            <span className="pointer-events-none absolute left-1/2 top-full z-10 mt-2 w-max -translate-x-1/2 rounded-xl bg-slate-950 px-3 py-1 text-[10px] font-bold text-white opacity-0 shadow-xl transition-opacity peer-hover:opacity-100">
